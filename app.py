...
def train_model(df, target_column, features, model_type):
    ...
    if df is None or df.empty or target_column not in df.columns or not features:
    ...
    try:
        ...
        if len(y_stratify.unique()) > 1 and min_class_count >= 2:
             X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42, stratify=y_stratify)
        else:
             X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)
             st.warning("Skipping stratified split due to small/imbalanced classes.")
...
def run_kmeans(df, features, n_clusters):
    ...
    if X.empty or X.shape[0] < n_clusters:
        st.error(f"Cannot run K-Means: Need at least {n_clusters} non-missing samples for clustering.")
        return None, None, None, None
...
def generate_report(df, predictions, model_results, plot_buffer, kmeans_results=None):
    # ... (Report generation function remains the same) ...
    pdf = FPDF()
    pdf.add_page()
...
# --- Sidebar Inputs (Still needed for preprocessing) ---
st.sidebar.subheader("Input Variables (Global)")
if st.session_state['processed_df'] is not None:
    all_columns = st.session_state['processed_df'].columns.tolist()
    
    # ... (Location, temporal, and pollution indicator selection logic remains, as these drive encoding)
    ...
    # Update state and rerun preprocessing
...
if page == "Home":
    ...
    st.markdown("All data mining models now run **automatically** using all suitable features.")

elif page == "Upload Dataset":
    # ... (Upload Dataset page remains the same) ...
    ...
    # ... (Data Analysis page remains the same) ...
    ...
    st.title("Prediction Dashboard (Automatic Classification)")

    if st.session_state['processed_df'] is not None:
        df = st.session_state['processed_df']

        st.subheader("Automatic Model Configuration (Random Forest)")
        st.info("The system automatically identifies a suitable target variable and uses all other compatible features.")

        # --- AUTOMATIC TARGET & FEATURE SELECTION ---
        
        # 1. Identify Candidate Features (All numeric + all encoded columns)
        available_cols = [col for col in df.columns if col not in [st.session_state.get('location_col'), st.session_state.get('temporal_col')]]
        numeric_features = [col for col in available_cols if pd.api.types.is_numeric_dtype(df[col])]
        encoded_features = st.session_state['encoded_features']
        all_valid_features = sorted(list(set(numeric_features + encoded_features)))
        
        # 2. Automatically select Target Column (Prefer a non-numeric, low-cardinality column, or the last non-feature column)
        target_candidates = [col for col in available_cols if col not in all_valid_features]
        if not target_candidates:
             # If all non-location/temporal are numeric/encoded, use a low-cardinality numeric column
             target_candidates = [col for col in available_cols if df[col].nunique() < 20]
             
        if target_candidates:
            # Pick the first suitable candidate and remove it from feature list
            target_column = target_candidates[0]
            if target_column in all_valid_features: # Remove if it was mistakenly included
                all_valid_features.remove(target_column) 
        else:
            st.error("Could not automatically determine a suitable target variable. Cannot run classification.")
            target_column = None
            feature_columns = None
            
        st.session_state['target_col'] = target_column 
        feature_columns = [col for col in all_valid_features if col != target_column]
        
        st.write(f"**Target Variable Selected:** `{target_column}`")
        st.write(f"**Features Used ({len(feature_columns)}):** `{', '.join(feature_columns)}`")
        
        model_type = "Random Forest"
        st.session_state['model_type'] = model_type 

        # --- AUTOMATIC EXECUTION ---
        if st.button("Run Automatic Classification & Show Results"):
            if target_column and feature_columns:
                with st.spinner(f"Training {model_type} model automatically..."):
                ...
                st.warning("Automatic classification failed: Check if data is loaded and has multiple usable columns.")
        ...
        if st.session_state['model'] is not None and st.session_state['predictions'] is not None and st.session_state['model_type'] in ["Random Forest", "XGBoost"]:
            ...
            if st.session_state['model_report']:
             ...
             st.info("Click the button above to run the automatic classification model.")
...
# --- K-Means Clustering Dashboard ---
    ...
    st.title("Clustering Dashboard (Automatic K-Means)")

    if st.session_state['processed_df'] is not None:
        df = st.session_state['processed_df']

        st.subheader("Automatic K-Means Configuration")
        st.info("The system uses **k=3** clusters and all compatible features (numeric/encoded).")

        # --- AUTOMATIC FEATURE SELECTION ---
        available_cols = [col for col in df.columns if col not in [st.session_state.get('location_col'), st.session_state.get('temporal_col')]]
        numeric_features = [col for col in available_cols if pd.api.types.is_numeric_dtype(df[col])]
        encoded_features = st.session_state['encoded_features']
        cluster_features = sorted(list(set(numeric_features + encoded_features)))
        n_clusters = 3 # Fixed cluster count
        
        st.write(f"**Number of Clusters (k):** `{n_clusters}`")
        st.write(f"**Features Used ({len(cluster_features)}):** `{', '.join(cluster_features)}`")

        st.session_state['model_type'] = 'K-Means'
        
        if st.button("Run Automatic Clustering & Show Results"):
            if cluster_features:
                with st.spinner(f"Running K-Means with k={n_clusters} automatically..."):
                ...
                st.warning("Automatic clustering failed: No suitable features found.")
        ...
        if st.session_state.get('kmeans_model') is not None and st.session_state.get('processed_df_clustered') is not None and st.session_state.get('model_type') == 'K-Means':
            ...
            st.info("Click the button above to run the automatic clustering model.")
            
    else:
        st.warning("Please upload and preprocess a dataset on the 'Upload Dataset' page first.")

elif page == "Reports":
    # ... (Reports page remains the same) ...
